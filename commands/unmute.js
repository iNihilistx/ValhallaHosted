...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!message.member.hasPermission('MANAGE_ROLES')) {
            return message.reply("You do not have permission to use the **Unmute** command!").then(m => m.delete({timeout: 2000}))
            message.delete()
        }
        ...
        if (!muteDoc) {
            ...
            return message.channel.send("Sunny does not have permission to use the unmute command for this member, their permissions the same or higher!")
        }
